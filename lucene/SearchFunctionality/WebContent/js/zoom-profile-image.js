...
function RemoveImage(){
    ...
    CloseImageEditor();
    $('.imgBox span.icon-Edit').off('click');
    $('.imgBox span.icon-Edit').attr("class","icon-Upload");
...
window.onload=function(){ 
 ...
 function ShowImageEditor(){
        ...
        //         SendImageTransformations();
        // });
    }
function CloseImageEditor(){
    $('#myModal').hide();$('.imgBox span.icon-Upload').click(function(){$('#imgInp').click();});
}
...
drawImageOnCanvas=function(){
   ...
   $("#canvas").scaleCanvas({
    x:  80, y: 80,
    scaleX: prevGlobalZoom==globalZoom ? 1: globalZoom
    ,layer:true
    }).rotateCanvas({
    rotate: prevGlobalRotate==globalRotate ? 0: globalRotate,
    x: 80, y: 80
    }).drawArc({
    mask: true,
    x: 80,y: 80,
    ...
    ,width:138
    ,height:138
    }).restoreCanvas({layer:true});
    prevGlobalZoom =globalZoom;
    prevGlobalRotate=globalRotate
    $('#profileImage').attr({'src':document.getElementById("canvas").toDataURL(),'style': " width:250%; height: 135% ;margin-top: -30px;margin-left: -23px;"});
...
function UploadImageClick(){
      $('.imgBox span.icon-Upload,.imgBox span.icon-Edit').hover(function(){
        $(this).css("cursor","pointer") ;
            });
        $('.imgBox span.icon-Upload').click(function(){
            $("#imgInp").click();
            });
...
function EditImageClick(){
                $('.imgBox span.icon-Upload').off('click');
                $('.imgBox div.photo-upload').attr("style","overflow: hidden;");
                $('.imgBox span.icon-Upload').attr("class","icon-Edit");    
                $('.imgBox span.icon-Edit').click(function(){
                $("#draggable").draggable({ containment: "#containment" });
                ShowImageEditor();
                    });          
}

// function SetLocalStorageImage(){
//     $("#profileImage").removeAttr('style','width');
//     $("#profileImage").attr({'src':localStorage.getItem("src"),'style': " width:250%; height: 135% ;margin-top: -30px;margin-left: -23px;"});
//      $("#modalImage").attr({'src':localStorage.getItem("originalImage")});
// }